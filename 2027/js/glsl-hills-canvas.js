/**
 * ALICARI 2027 - Authentic GLSL Hills 3D Shader
 * Direct WebGL Three.js implementation of the Alicari red/white undulating dot topography.
 */

export function initGLSLHills(canvasId = 'glsl-hills-canvas') {
        ...
        new THREE.RawShaderMaterial({
          ...
          fragmentShader: `
            ...
            void main(void) {
              ...
              // Dot pattern
              vec2 grid = fract(vPosition.xz * 2.0);
              float dist = length(grid - vec2(0.5));
              
              float sizeThreshold = 0.12 + (vMouseInfluence * 0.14);
              float dotShape = smoothstep(sizeThreshold, sizeThreshold - 0.07, dist);
              
              // Checkerboard pattern for alternating colors
              float checker = step(0.5, fract((floor(vPosition.x * 2.0) + floor(vPosition.z * 2.0)) * 0.5));
              
              vec3 colorRed = vec3(0.988, 0.133, 0.133); // #fc2222
              vec3 colorWhite = vec3(1.0, 1.0, 1.0);
              
              vec3 finalColor = mix(colorWhite, colorRed, checker);
              
              gl_FragColor = vec4(finalColor, opacity * dotShape * 2.2);
            }
          `,
          transparent: true
        })
